...
(function (document, window, console) {
    ...
    function init() {
        ...
        document.addEventListener('DOMContentLoaded',
            function () {
                document.body.insertAdjacentHTML('beforeend', '<input id="size" type="range" min="1" value="3" max="14"/>');
                let inputSize = document.querySelector('input#size');
                inputSize.addEventListener('change',function(e){
                    clear(args);
                    addSeed(args);   
                    param.stepLimit = parseInt(e.target.value,10);
                    main(0, args);
                });
    ...
    function addSeed (args) {
        if (typeof g_main !== 'undefined') {
            polygonSVG(g_main, config.stage.width / 3, config.stage.height / 8, 0, 1, 1, 0, args);
        } else {
            polygonSVG(0, config.stage.width / 3, config.stage.height / 8, 0, 1, 1, 0, args);            
        }
    }
