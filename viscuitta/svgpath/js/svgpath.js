...
export const transformParse = (function () {
  ...
  let CMD_SPLIT_RE = /\s*(matrix|translate|scale|rotate|skewX|skewY)\s*\(\s*(.+?)\s*\)[\s,]*/;
...
export const Ellipse = (function () {
  ...
  Ellipse.prototype.transform = function (m) {
    ...
    let c = Math.cos(this.ax * torad), s = Math.sin(this.ax * torad);
    let ma = [
      this.rx * (m[0] * c + m[2] * s),
      this.rx * (m[1] * c + m[3] * s),
      this.ry * (-m[0] * s + m[2] * c),
